...
let rec g env = function (* α変換ルーチン本体 (caml2html: alpha_g) *)
  ...
  | Let((x, t), e1, e2) -> (* letのα変換 (caml2html: alpha_let) *)
      let x' = Id.genid (Id.to_string x) in
      Let((x', t), g env e1, g (M.add x x' env) e2)
  | Var(x) -> Var(find x env)
  | LetRec({ name = (x, t); args = yts; body = e1; tags = _ }, e2) -> (* let recのα変換 (caml2html: alpha_letrec) *)
  ...
  | Assign(x, y, e) -> Assign(find x env, find y env, g env e)
