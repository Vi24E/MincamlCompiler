...
let default_tag () = ref Unknown

let rec fv = function (* 式に出現する（自由な）変数 (caml2html: knormal_fv) *)
  | Unit | Int(_) | Float(_) | ExtArray(_) -> S.empty
  | Neg(x) | FNeg(x) -> S.singleton x
  | Add(x, y) | Sub(x, y) | Sll(x, y) | Sra(x, y) | FAdd(x, y) | FSub(x, y) | FMul(x, y) | FDiv(x, y) | Get(x, y) -> S.of_list [x; y]
  | IfEq(x, y, e1, e2) | IfLE(x, y, e1, e2) -> S.add x (S.add y (S.union (fv e1) (fv e2)))
  | Let((x, _), e1, e2) -> S.union (fv e1) (S.remove x (fv e2))
  | Var(x) -> S.singleton x
  | LetRec({ name = (x, _); args = yts; body = e1; tags = _}, e2) ->
...
let rec log2 i n =
  if i = 1 then n
  else if i mod 2 = 0 then log2 (i / 2) (n + 1)
  else -1
...
let rec g env = function (* K正規化ルーチン本体 (caml2html: knormal_g) *)
  ...
  | Syntax.Mul(e1, e2) -> 
      (match e2 with
       | Syntax.Int(i) ->
           ...
           if p >= 0 then
             insert_let (g env e1)
               (fun x -> insert_let (g env (Syntax.Int(p)))
                   (fun y -> Sll(x, y), Type.Int))
           else
             insert_let (g env e1)
               (fun x -> insert_let (g env e2)
                   (fun y -> ExtFunApp(("min_caml_mul", Id.Unknown), [x; y]), Type.Int))
       | _ -> 
           insert_let (g env e1)
             (fun x -> insert_let (g env e2)
                 (fun y -> ExtFunApp(("min_caml_mul", Id.Unknown), [x; y]), Type.Int)))
      ...
      (match e2 with
       | Syntax.Int(i) ->
           ...
           if p >= 0 then
             ...
             insert_let (g env e1)
               (fun x -> insert_let (g env e2)
                   (fun y -> ExtFunApp(("min_caml_div", Id.Unknown), [x; y]), Type.Int))
       | _ -> 
           insert_let (g env e1)
             (fun x -> insert_let (g env e2)
                 (fun y -> ExtFunApp(("min_caml_div", Id.Unknown), [x; y]), Type.Int)))
  ...
  | Syntax.LetRec({ Syntax.name = (x, t); Syntax.args = yts; Syntax.body = e1 }, e2) ->
      ...
      LetRec({ name = (x, t); args = yts; body = e1'; tags = default_tag()}, e2'), t2
          ...
          insert_let g_e2
            (fun y ->
              let l =
                ...
                | _ -> "create_array" in
              ExtFunApp((l, Id.Unknown), [x; y]), Type.Array(t2)))
  | Syntax.Get(e1, e2) ->
      (match g env e1 with
